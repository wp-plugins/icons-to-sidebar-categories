<?php
/*
Plugin Name: Icons to sidebar after categories
Plugin URI:
...
if ( isset($action) ) {
    ...
    switch ( $action ) :
        ...
        case 'delete-image-from-htmltable' :


            $path_icon = $_POST['file_name_dir'] . '/' . $_POST['file_name'];

            $name_parts = pathinfo($path_icon);
...
function rss_filter($filter_type, $filter) {
...
function rsscp_filter($filter_type, $filter) {
    global $wpdev_cp_filter;
...
function icon_filter($filter_type) {
    global $wpdev_cp_filter;
...
function icon_cp_action($action_type) {
...
function icons_action($action_type, $action) {
    global $wpdev_cp_action;
...
function icon_remove_cp($action_type, $action) {
    global $wpdev_cp_action;
...
if (!function_exists ('icon_icon_debuge')) {
    function icon_icon_debuge() {
...
function icon_data_mc_wpdev( $file, $default_headers, $context = '' ) {
...
function icons_define_static() {
    ...
    $plugin_data = icon_data_mc_wpdev(  __FILE__, $default_headers, 'plugin' );
    if ( $plugin_data['Name'] == 'Icons-Category-Page')  $isPublic = 1;
    else                                                                  $isPublic = 0;


    if (!defined('WPDEV_CP_VERSION'))    define('WPDEV_CP_VERSION',  $plugin_data['Version'] );
    if (!defined('WPDEV_CP_PUBLIC_VERSION'))    define('WPDEV_CP_PUBLIC_VERSION', $isPublic );
    if (!defined('WP_CONTENT_DIR'))      define('WP_CONTENT_DIR', ABSPATH . 'wp-content');
    if (!defined('WP_CONTENT_URL'))      define('WP_CONTENT_URL', get_option('siteurl') . '/wp-content');
    if (!defined('WP_PLUGIN_DIR'))       define('WP_PLUGIN_DIR', WP_CONTENT_DIR . '/plugins');
    if (!defined('WP_PLUGIN_URL'))       define('WP_PLUGIN_URL', WP_CONTENT_URL . '/plugins');
    if (!defined('WPDEV_CP_PLUGIN_FILENAME'))  define('WPDEV_CP_PLUGIN_FILENAME',  basename( __FILE__ ) );
    if (!defined('WPDEV_CP_PLUGIN_DIRNAME'))   define('WPDEV_CP_PLUGIN_DIRNAME',  plugin_basename(dirname(__FILE__)) );
    if (!defined('WPDEV_CP_PLUGIN_DIR')) define('WPDEV_CP_PLUGIN_DIR', WP_PLUGIN_DIR.'/'.WPDEV_CP_PLUGIN_DIRNAME );
    if (!defined('WPDEV_CP_PLUGIN_URL')) define('WPDEV_CP_PLUGIN_URL', WP_PLUGIN_URL.'/'.WPDEV_CP_PLUGIN_DIRNAME );
...
if (!class_exists('icon_icon_wpdev_compose')) {
    class icon_icon_wpdev_compose {
        ...
        function icon_icon_wpdev_compose() {
            ...
            add_action('admin_menu', array(&$this,'icon_add_new_admin_menu'));
            ...
            register_activation_hook( __FILE__, array(&$this,'icon_icon_wpdev_compose_activate' ));
            register_deactivation_hook( __FILE__, array(&$this,'icon_icon_wpdev_compose_deactivate' ));
        ...
        function icon_add_new_admin_menu() {
            ...
            if (! WPDEV_CP_PUBLIC_VERSION) {
                $pagehook1 = add_object_page( __('Menu compouser'), __('Site Menu'), $users_roles[0],
                        __FILE__ . 'wpdev-cp', array(&$this, 'on_show_icon_page_menucompouser'),  WPDEV_CP_PLUGIN_URL . '/img/Sidebar-Photos-16x16.png'  );
            ...
            if (WPDEV_CP_PUBLIC_VERSION) {
                $pagehook2 = add_object_page(__('Uploading icons for pages and categories', 'wpdev-booking'), __('Icons', 'wpdev-booking'), $users_roles[1],
                        __FILE__ .'wpdev-cp-icons', array(&$this, 'on_show_cp-icon_page_icons'),  WPDEV_CP_PLUGIN_URL . '/img/Sidebar-Photos-16x16.png'    );
                ...
                $pagehook2 = add_submenu_page(__FILE__ . $reference_for_submenu,__('Uploading icons for pages and categories', 'wpdev-booking'), __('Icons', 'wpdev-booking'), $users_roles[1],
                        __FILE__ .'wpdev-cp-icons', array(&$this, 'on_show_cp-icon_page_icons')  );
            }
...
<div id="<?php echo $html_id; ?>-general" class="wrap">
    ...
    <h2><?php echo $title; ?></h2>
        ...
        function get_menu_array() {
            ...
            if ($this->menu !== false) return $this->menu;
...
//icon_icon_debuge($final_sorted_array_keys);
            ...
            foreach ($my_childs as $pg) {
                ...
                if (! empty ($pg['icon']) ) {
                    ...
                    if ( $pos !== false) {
                        ...
                        $pos = icon_filter('wpdev_pos_img_page',$pos, $output);
                        $beforeImg = icon_filter('wpdev_before_img_page','');
                        $afterImg  = icon_filter('wpdev_after_img_page','');
                    ...
                    foreach ($my_childs as $pg) {
                        ...
                        if (! empty ($pg['icon']) ) {
                            ...
                            if ( $pos !== false) {
                                ...
                                $pos = icon_filter('wpdev_pos_img_category',$pos, $output);
                                $beforeImg = icon_filter('wpdev_before_img_category','');
                                $afterImg  = icon_filter('wpdev_after_img_category','');
                        ...
                        <div class="inside">
                            <form  name="post_option" action="" method="post" id="post_option" >
                                <table class="form-table"><tbody>
                                        ...
                                        <tr valign="top">
                                            <th scope="row"><label for="wpdev_mc_icon_dir" ><?php _e('Store uploads of icons in this folder1'); ?>:</label></th>
                                            <td><input id="wpdev_mc_icon_dir" class="regular-text code" type="text" size="45" value="<?php echo $wpdev_mc_icon_dir; ?>" name="wpdev_mc_icon_dir"/>
                                                <span class="description"><?php printf(__('Default is %s'),'<span><b>wp-content/uploads/icons    : If need nice icon visit http://www.iconfinder.com </b></span>');?></span>
                                            </td>
                                        </tr>
                    ...
                    if (! empty($top_menu['selected'])) {                            // cehck if show submenu at entered link
                        if ( count($top_menu['selected']['childs']) > 0 ) {      // check if some childs of submenu here
                            ...
                            for ($i = 0 ; $i < $ic ; $i++) {
                                ...
                                if (! empty($page['icon']) )
                                    $my_submenu .=  '<img  width="32" height="32"  alt="'.$page['title'].'" src="'. /*$this->icons_url .'/'. */$page['icon'].'"/>';  //TODO: SET DIRECTORY TO ICONS
        ...
        // Activate
        function icon_icon_wpdev_compose_activate() {
            ...
            add_option('wpdev_mc_menu_links'  , '|wordpress|wp-plugins|startups|portfolio|mailto:info@emsphere.info');
            add_option('wpdev_mc_submenu_id'  , 'page=3|cat=4|cat=5|cat=6|cat=7|');/**/
            add_option('wpdev_mc_icon_size_w' , '32');
            add_option('wpdev_mc_icon_size_h' , '32');
            ...
            icon_cp_action('wpdev_mc_activation');


            $recheck = get_option('wpdev_mc_icon_dir');
            if ($recheck == '/icons') {
                add_option('wpdev_mc_icon_dir'    , 'wp-content/uploads/1/icons'); /**/
            }
        }

        // Deactivate
        function icon_icon_wpdev_compose_deactivate() {
            ...
            if ($is_delete && ( $is_delete_if_deactive == 'On' ) ) {
                ...
                icon_cp_action('wpdev_mc_deactivation');
            }
        }
...
$wpdev_mc = new icon_icon_wpdev_compose();
